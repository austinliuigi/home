...
function collect_networks() {
    ...
    echo -ne "$scan";\
    echo -ne "\0icon\x1f<span color='white'></span>\n";\
    echo -ne "$refresh";\
    echo -ne "\0icon\x1f<span color='white'></span>\n";\
    ...
    while IFS= read -r line; do
        ...
        echo -ne "$network";\
        # echo -ne "\0display\x1f${strength} ${network}";\ # requires commit dbc1f8d
        echo -ne "\0icon\x1f<span color='white'>${icon}</span>\n";\
    done <<< "$network_dump"
}


while [ true ]; do
    network="$(collect_networks | rofi -dmenu -i -p "network")"
