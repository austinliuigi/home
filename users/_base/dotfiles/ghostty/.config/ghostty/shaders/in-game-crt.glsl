...
// [0, 1]
#define DARKEN_MIX 0.1

// How far in from the border the vignette spreads
// x \in R : x >= 0
#define VIGNETTE_SPREAD 0.175
// How bright the vignette is
// x \in R : x >= 0
#define VIGNETTE_BRIGHTNESS 2.75
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // - we wrap iTime in mod() because otherwise, the noise forms rings like a tree trunk after a period of time for unknown reasons
    float noiseContent = smoothstep(0.4, 0.6, fract(sin(uv.x * uv.y * (1.0-uv.x) * (1.0-uv.y) * mod(iTime, 10.0) * 4096.0) * 65536.0));
    float noiseUniform = smoothstep(0.4, 0.6, fract(sin(uv.x * uv.y * (1.0-uv.x) * (1.0-uv.y) * mod(iTime, 10.0) * 8192.0) * 65536.0));
