...
while [ true ]; do
    ...
    network="$(echo -ne "$network_entries" | fzf --no-clear --prompt="network: ")"
...
if [ "$(echo "$known_networks" | grep -c "$network")" -gt 0 ]; then
    ...
    iwctl station wlan0 connect "$network" --passphrase "$(fzf --prompt="passphrase: " --info=hidden --print-query </dev/null)"
    notify
fi
